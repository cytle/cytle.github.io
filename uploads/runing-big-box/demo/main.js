$(function(){
	var n=6;
			...
			function set_game_speed(){ // 移动蛇 
				...
				for(var i=0;i<map.length;i++){ if(map[i].face="=face.now&&" parseint(map[i].x)="=x" &&="" parseint(map[i].y)="=y){" alert("你挂了，继续努力吧！失败原因：撞到自己了.....");window.location.reload();="" }="" if="" (map.length="">t) { //保持舍身长度 
			...
			function changeFace(){
				...
				if(x>=width){
					...
					x=0;
				}else if(x<=0){ ff="face.now;" face.now="face.left;" face.left="face.dm;" face.dm="face.right;" face.right="ff;" x="width;" }else="" if((y="">=width)){
					...
					y=0;
				}else if((y<=0)){ ff="face.now;" face.now="face.top;" face.top="face.dm;" face.dm="face.bottom;" face.bottom="ff;" y="width;" }="" var="" running="true;" document.onkeydown="function(e)" {="" 改变蛇方向="" $(".log").html($(".log").html()+"<br="">x:"+direction);
				if(e.keyCode==32){
					if(running){
